...
class bound_misbound_XII : public model_kmc {
	...
	protected:	
		...
		vector <vector <int> > occupy_list;
		vector <vector <int> > occupy_list_location;
		vector <int> add_cluster;
	...
	public:
		bound_misbound_XII(int i_number_monomers) {
			...
			occupy_list_location[0][0] = 0;
			
		}
		~bound_misbound_XII() {;} 
		void create_vectors() {
			clusters.resize(number_monomers+1);
			occupy_list_location.resize(number_monomers+1);
			for(int i = 0; i < number_monomers+1; i++) {
				clusters[i].resize(number_monomers+1,0);
				occupy_list_location[i].resize(number_monomers+1,-1);
			}
